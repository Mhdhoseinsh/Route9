            ...
            window.FBRoom = {
                ...
                onMessage(callback) {
                    const handler = (msg) => {
                        ...
                        // sender's payload claims. `seq` is a per-SENDER,
                        // ever-increasing counter (each slot has its own
                        // count) so the caller can notice a missed message
                        // from one specific opponent (network hiccup, etc.)
                        // and ask for a fresh state sync instead of silently
                        // drifting out of sync — without false-triggering
                        // just because a different player also sent a
                        // message in between.
